...
| [Windows-Unattended-Provisioning](https://github.com/diogowermann/Windows-Unattended-Provisioning) | Security-conscious Windows provisioning pipeline that builds unattended setup media, joins endpoints to a defined on-premises Active Directory OU, completes Hybrid Microsoft Entra registration, and prepares the device for Intune-managed Windows LAPS. | Unattended deployment, build-time secret injection, domain join, hybrid identity, sensitive-artifact cleanup |
